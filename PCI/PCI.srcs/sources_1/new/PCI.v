...
always @ (posedge CLK)
    if (RST_N)
	begin
		if((!GNT_N && !REQ_N && BusStatus) || MasterFlag ) //Then Device is Master
		begin 
			MasterFlag = 1;
			case (Status)
			TransactionStart:
					...
					begin
						...
						begin
							if(CBE_N [0] == 0)
								memory[0][7:0]  <= AD [7:0];
							if(CBE_N [1] == 0)
								memory[0][15:8] <= AD [15:8];
							if(CBE_N [2] == 0)
								memory[0][23:16] <= AD [23:16];
							if(CBE_N [3] == 0)
...
begin
    ...
    else if((FRAME_Neg && EN) || (BusStatus))
    begin
        casez(REQ_Neg)
            8'bzzzz_zzz0:begin GNT_Neg <= 8'b1111_1110; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bzzzz_zz01:begin GNT_Neg <= 8'b1111_1101; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bzzzz_z011:begin GNT_Neg <= 8'b1111_1011; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bzzzz_0111:begin GNT_Neg <= 8'b1111_0111; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bzzz0_1111:begin GNT_Neg <= 8'b1110_1111; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bzz01_1111:begin GNT_Neg <= 8'b1101_1111; EN <= 1'b0;BusStatus <= 1'b0; end
            8'bz011_1111:begin GNT_Neg <= 8'b1011_1111; EN <= 1'b0;BusStatus <= 1'b0;end
            8'b0111_1111:begin GNT_Neg <= 8'b0111_1111; EN <= 1'b0;BusStatus <= 1'b0; end
            default:begin GNT_Neg <= 8'b1111_1111; EN <= 1'b1;BusStatus <= 1'b1; end
        endcase
    end
end
endmodule
...
module PCI(CLK,RST_N, FORCED_REQ_N, FORCED_ADDRESS, FORCED_CBE_N, Forced_DataTfNo,grant);
...
arbiter_FCFS arbiter1(GNT_N , REQ_N , FRAME_N,CLK, RST_N,1);
assign grant=GNT_N;
endmodule
...
module PCI_tb();
 ...
 PCI pci(CLK,RST_N,FORCED_REQ_N, FORCED_ADDRESS, FORCED_CBE_N, Forced_DataTfNo,grant);
